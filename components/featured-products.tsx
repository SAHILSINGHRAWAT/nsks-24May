    ...
    <section className="py-12 px-4 md:px-6 lg:px-8 bg-gray-50">
      <div className="container mx-auto max-w-6xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <h2 className="text-3xl font-extrabold tracking-tight mb-2 text-black drop-shadow-sm">Featured Products</h2>
            <p className="text-black">Explore our most popular cable management solutions</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {featuredProducts.map((product) => (
            <div key={product.id} className="bg-white border rounded-lg overflow-hidden shadow-md group transition-transform duration-300 hover:scale-105 hover:shadow-xl cursor-pointer">
              <Link href={`/products/${product.id}`} className="block">
                ...
                <div className="p-4">
                  <h3 className="font-bold mb-2 text-black text-lg group-hover:underline transition-colors duration-200">{product.name}</h3>
